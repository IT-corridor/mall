...
class JsApi_pub(Common_util_pub):
    ...
    def get_access_tocken(self):
        url = "https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=%s&secret=%s"%(WxPayConf_pub.APPID, WxPayConf_pub.APPSECRET)
        print url
        data = HttpClient().get(url)
        print data
        return data

    def get_signature(self, url, ticket):
        jsApiObj = {}
        jsApiObj["noncestr"] = self.createNoncestr()
        #jsApiObj["noncestr"] = "Wm3WZYTPz0wzccnW"  #  用于测试weixin示例签名
        timeStamp = int(time.time())
        jsApiObj["timestamp"] = "{0}".format(timeStamp)
        #jsApiObj["timestamp"] = "1414587457"
        jsApiObj["jsapi_ticket"] = "{0}".format(ticket)
        jsApiObj["url"] = "{0}".format(url)
        #签名步骤一：按字典序排序参数,formatBizQueryParaMap已做
        String = self.formatBizQueryParaMap(jsApiObj, False)
        #print String
        String = hashlib.sha1(String).hexdigest()
        jsApiObj["signature"] = String
...
class UnifiedOrder_pub(Wxpay_client_pub):
    ...
    def getPrepayId(self):
        ...
        self.postXml()
        self.result = self.xmlToArray(self.response)
        print "code_url is here:", self.result
        prepay_id = self.result["prepay_id"]
        code_url = self.result.get("code_url", None)
...
def get_xml_key(text, key):
    root = ET.fromstring(text)

    # 获取element的方法
    # 1 通过getiterator
    value = None
    lst_node = root.getiterator(key)
...
def read_xml(text):
    ...
    for node in lst_node:
        out_trade_no = node.text


    lst_node = root.getiterator("result_code")
    for node in lst_node:
        return_code = node.text
    return return_code, out_trade_no
...
def test():
    #a = JsApi_pub()
    #a.get_signature("http://mp.weixin.qq.com?params=value", "sM4AOVdWfPE4DxkXGEs8VMCPGGVi4C3VM0P37wVUCFvkVAy_90u5h9nbSlYy3-Sl-HhTdfl2fzFy1AOcHKP7qg")
    b = OrderQuery_pub()
    b.setParameter("out_trade_no", "1431778865")
    b.setParameter("transaction_id", "1001390775201505160134773470")
    response = b.postXml()
    print response
    read_xml(response)
    #c = HttpClient()
